...
def _update_cache() -> None:
    ...
    try:
        ...
        for world in AutoWorldRegister.world_types.values():
            cls = None
            world_module = sys.modules.get(world.__module__)
            if world_module:
                # Pass 1: look for subclasses of Group
                for attr_name in dir(world_module):
                    val = getattr(world_module, attr_name)
                    if isinstance(val, type) and issubclass(val, Group) and val is not Group:
                        cls = val
                        break
                # Pass 2: fallback to any class ending with Settings
                if cls is None:
                    for attr_name in dir(world_module):
                        val = getattr(world_module, attr_name)
                        if isinstance(val, type) and val.__name__.endswith("Settings") and val is not Group:
                            cls = val
                            break
            if cls is not None:
...
class Settings(Group):
    ...
    def __getattribute__(self, key: str) -> Any:
        if key.startswith("_") or key in self.__class__.__dict__:
            ...
            cls = None
            try:
                cls_or_name = getattr(world, "__annotations__", {})["settings"]
            ...
            if cls is None:
                world_module = sys.modules.get(world_mod)
                if world_module:
                    # Pass 1: look for subclasses of Group
                    for attr_name in dir(world_module):
                        val = getattr(world_module, attr_name)
