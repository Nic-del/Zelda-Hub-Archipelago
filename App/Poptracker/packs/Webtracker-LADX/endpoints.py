...
def getDiskSettings(prefix='', jsonify=True):
    ...
    if 'localStorage' in diskSettings:
        settings['localStorage'] = diskSettings['localStorage']
...
def getSidebarMessage():
    try:
        ...
        return None
jsonEndpoints = {'/playerState', '/eventInfo', '/createEvent', '/checks', '/vueInit', '/diskSettings', '/api/saveWindowState' }
corsEndpoints = {'/playerState', '/playerId', '/suggestion', '/eventInfo', '/createEvent', '/event', '/checks', '/vueInit', '/items', '/checkList', '/shortString', '/spoilerLog', '/diskSettings', '/api/saveWindowState' }
@app.after_request
...
@app.route("/api/saveWindowState", methods=['POST'])
def saveWindowState():
    if not app.config['local']:
        return "OK"
...
def vueInit():
    ...
    if app.config.get('apServer'):
        settingsOverrides['apServer'] = app.config['apServer']
        settingsOverrides['autoConnectAp'] = True
    if app.config.get('apSlot'):
        settingsOverrides['apSlot'] = app.config['apSlot']
        settingsOverrides['autoConnectAp'] = True
    if app.config.get('apPassword'):
        settingsOverrides['apPassword'] = app.config['apPassword']
