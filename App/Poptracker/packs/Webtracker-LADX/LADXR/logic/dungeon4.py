...
class Dungeon4:
    def __init__(self, options, world_setup, r):
        ...
        if options.logic == 'hell':
            ...
            south_crossroads.connect(south_crossroads_chest7, AND(r.jesus_buffer, HOOKSHOT), back=False) # boots bonk across the water bottom wall to the bottom left corner, then hookshot up #TODO: replace with hookshot-only method:
            #TODO: south_crossroads.connect(south_crossroads_chest7, AND(r.jesus_buffer_hookshot, r.hookshot_clip_block), back=False) # hookshot the pot to stand atop chest. repeat until clipped into pot enough to walk through diagonally. can pause after hookshot animation to jesus buffer back onto chest anytime. recommend s&q menu during hookshot animation to act as frame-advance
            north_crossroads.connect(before_miniboss, AND(OR(r.shaq_jump, BOMB, BOW), r.hookshot_clip_block), back=False) # REMOVE and replace with below to add super bump
            #TODO: north_crossroads.connect(before_miniboss, OR(AND(OR(r.shaq_jump, BOMB, BOW), r.hookshot_clip_block), AND(r.shaq_jump, r.super_bump)), back=False) # push block left of keyblock up, then shaq jump off the left wall, push another block up, and hookshot clip through | split zol for more entities, and clip through the block left of keyblock by hookshot spam | shaq jump off pushblock, then shield bump/ pause buffer in such a way to land in push block
            #TODO: before_miniboss.connect(north_tile_puzzle, AND(r.jesus_buffer, r.jesus_buffer_hookshot), back=False) # boots bonk and jesus buffer to screen transition, s&q menu to stop drowning and buffer hookshot to get back on land
            #TODO: before_miniboss.connect(south_tile_puzzle, AND(r.jesus_buffer, r.jesus_buffer_hookshot) # bonk on wall above puddle chest, buffer all the way to screen transition, hookshot can get you above water after screen transition to restart buffer [stair shuffle prep]
            ...
            #TODO: outside_b_passage_keyblock.connect(south_tile_puzzle, r.jesus_buffer, back=False) # start a buffer by the button to get to the shallow water below [stair shuffle prep]
