...
export class APClientManager {
    ...
    async login(
        ...
        client.socket.on('dataPackage', (content) => {
            ...
            if (ssData !== undefined) {
                ...
                // Process queued items now that we have the mappings
                if (this.itemQueue.length > 0) {
                    for (const itemId of this.itemQueue) {
                        this.processItemById(itemId);
                    }
                    this.itemQueue = [];
                    this.resolveItems?.(this.inventory);
                }
        ...
        client.socket.on('receivedItems', (content) => {
            ...
            for (const netItem of content.items) {
                if (!this.idToItem) {
                    this.itemQueue.push(netItem.item);
                    continue;
                }
                if (this.processItemById(netItem.item)) {
                    changed = true;
                }
            }
            if (changed) {
        ...
        try {
            ...
            await client.login(server, slot, 'Skyward Sword', {
                tags: ['Tracker'],
                password: password,
                items_handling: 7, // Receive all items (starting + yours + others)
            } as any);
            this.client = client;
            this.status = {
                state: 'loggedIn',
    ...
    private processItemById(itemId: number): boolean {
